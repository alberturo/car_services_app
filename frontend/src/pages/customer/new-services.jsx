  ...
  useEffect(() => {
    async function getVehicals(id) {
      const response = await axios.get(
        "http://localhost:8080/api/vehicals?id=" + id,
        {
          headers: {
            Authorization:
              "Bearer " + JSON.parse(localStorage.getItem("token")),
            ...
            <form onSubmit={submitHandler} className="space-y-4">
              ...
              <div className="flex">
                ...
                >
                  {vehicals.map((vehical) => (
                    <option key={vehical.id} value={vehical.id}>
                      {vehical.type}
                    </option>
                  ))}
                </select>
              ...
              <div className="mt-4">
                <button
                  ...
                  type="submit"
                >
                  <span className="absolute inset-0 bg-gradient-to-r from-yellow-500 to-blue-500 transition-transform group-hover:translate-y-0 group-hover:translate-x-0 rounded-full"></span>

                  <span className="relative inline-block border-2 border-current px-5 py-2 text-sm font-bold uppercase tracking-widest text-white bg-gradient-to-r from-yellow-500 to-blue-500 rounded-full shadow-lg group-active:text-opacity-75 transition-transform transform hover:scale-105">
                    Add a New Service
                  </span>
                </button>
              </div>
